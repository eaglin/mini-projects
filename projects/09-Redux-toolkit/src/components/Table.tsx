...
import { deleteUserById } from "../store/users/slice";
  ...
  export default function TableTremor() {

    const users = useAppSelector((state:RootState) => state.user)
    const dispatch = useAppDispatch();
    

    const handleClick = (id:string)=>   dispatch(deleteUserById(id))
            ...
            {users.map((item) => (
              <TableRow key={item.name}>
                <TableCell className="text-right">
                    <img style={{height:'24px',width:'24px'}} src={`https://unavatar.io/github/${item.github}`}></img>
                    {item.name}</TableCell>
                <TableCell className="text-right">{item.email}</TableCell>
                <TableCell className="text-right">{item.github}</TableCell>
                <TableCell className="text-right" style={{justifyContent:'space-between'}}><button>
